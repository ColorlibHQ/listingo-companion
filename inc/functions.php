...
// Section Heading
function listingo_section_heading( $title = '', $subtitle = '' ) {
	if( $title || $subtitle ) :
	?>
		<div class="container">
			<div class="row">
...
// Enqueue scripts
add_action( 'wp_enqueue_scripts', 'listingo_companion_frontend_scripts', 99 );
function listingo_companion_frontend_scripts() {

	wp_enqueue_script( 'listingo-companion-script', plugins_url( '../js/loadmore-ajax.js', __FILE__ ), array( 'jquery' ), '1.0', true );
	wp_enqueue_script( 'listingo-common-js', plugins_url( '../js/common.js', __FILE__ ), array( 'jquery' ), '1.0', true );

}
// 
add_action( 'wp_ajax_listingo_portfolio_ajax', 'listingo_portfolio_ajax' );

add_action( 'wp_ajax_nopriv_listingo_portfolio_ajax', 'listingo_portfolio_ajax' );
function listingo_portfolio_ajax( ){
	...
	function listingo_update_the_followed_post_page_status( $title = 'Hello world!', $type = 'post', $status = 'draft', $message = false ){
...
function listing_custom_posts() {	
	$labels = array(
		'name'               => _x( 'Listing', 'post type general name', 'listingo-companion' ),
		'singular_name'      => _x( 'Listing', 'post type singular name', 'listingo-companion' ),
		'menu_name'          => _x( 'Listing', 'admin menu', 'listingo-companion' ),
		'name_admin_bar'     => _x( 'Listing', 'add new on admin bar', 'listingo-companion' ),
		'add_new'            => _x( 'Add New', 'listingo', 'listingo-companion' ),
		'add_new_item'       => __( 'Add New Listing', 'listingo-companion' ),
		'new_item'           => __( 'New Listing', 'listingo-companion' ),
		'edit_item'          => __( 'Edit Listing', 'listingo-companion' ),
		'view_item'          => __( 'View Listing', 'listingo-companion' ),
		'all_items'          => __( 'All Listings', 'listingo-companion' ),
		'search_items'       => __( 'Search Listing', 'listingo-companion' ),
		'parent_item_colon'  => __( 'Parent Listing:', 'listingo-companion' ),
		'not_found'          => __( 'No Listing found.', 'listingo-companion' ),
		'not_found_in_trash' => __( 'No Listing found in Trash.', 'listingo-companion' )
	);

	$args = array(
		'labels'             => $labels,
		'description'        => __( 'Description.', 'listingo-companion' ),
		'public'             => true,
	...
	$labels = array(
		'name'              => _x( 'Listing Category', 'taxonomy general name', 'listingo-companion' ),
		'singular_name'     => _x( 'Listing Categories', 'taxonomy singular name', 'listingo-companion' ),
		'search_items'      => __( 'Search Listing Categories', 'listingo-companion' ),
		'all_items'         => __( 'All Listings Categories', 'listingo-companion' ),
		'parent_item'       => __( 'Parent Listing Category', 'listingo-companion' ),
		'parent_item_colon' => __( 'Parent Listing Category:', 'listingo-companion' ),
		'edit_item'         => __( 'Edit Listing Category', 'listingo-companion' ),
		'update_item'       => __( 'Update Listing Category', 'listingo-companion' ),
		'add_new_item'      => __( 'Add New Listing Category', 'listingo-companion' ),
		'new_item_name'     => __( 'New Listing Category Name', 'listingo-companion' ),
		'menu_name'         => __( 'Listing Category', 'listingo-companion' ),
	);
	...
	$labels = array(
		'name'              => _x( 'Listing Country', 'taxonomy general name', 'listingo-companion' ),
		'singular_name'     => _x( 'Listing Countries', 'taxonomy singular name', 'listingo-companion' ),
		'search_items'      => __( 'Search Listing Countries', 'listingo-companion' ),
		'all_items'         => __( 'All Listings Countries', 'listingo-companion' ),
		'parent_item'       => __( 'Parent Listing Country', 'listingo-companion' ),
		'parent_item_colon' => __( 'Parent Listing Country:', 'listingo-companion' ),
		'edit_item'         => __( 'Edit Listing Country', 'listingo-companion' ),
		'update_item'       => __( 'Update Listing Country', 'listingo-companion' ),
		'add_new_item'      => __( 'Add New Listing Country', 'listingo-companion' ),
		'new_item_name'     => __( 'New Listing Country Name', 'listingo-companion' ),
		'menu_name'         => __( 'Listing Country', 'listingo-companion' ),
	);
...
/*=========================================================
    Cases Section
========================================================*/
function listingo_case_section( $post_order ){ 
	...
	if( $cases->have_posts() ) {
		while ( $cases->have_posts() ) {
			...
			$case_img      = get_the_post_thumbnail( get_the_ID(), 'listingo_case_study_thumb_362x240', '', array( 'alt' => get_the_title() ) );
			?>
...
// Related listingo for Single Page
function listingo_related_items( $current_post_id = null, $post_item = 3, $post_order = 'DESC', $have_related_listing_title = 'Related Listings' ){
			...
			<div class="row">
				...
				if( $related_listings->have_posts() ) {
					while ( $related_listings->have_posts() ) {
						...
						$recipe_img = get_the_post_thumbnail( get_the_ID(), 'listingo_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
						$listing_address = ! empty( listingo_meta( 'listing_address') ) ? listingo_meta( 'listing_address') : 'N/A';
						$phone_number = ! empty( listingo_meta( 'phone_number') ) ? listingo_meta( 'phone_number') : 'N/A';
						$listing_email = ! empty( listingo_meta( 'listing_email') ) ? listingo_meta( 'listing_email') : 'N/A';
						?>
...
function listingo_get_tabbed_contents( $sec_title = 'Explore Europe', $selected_countries ) {
	$i = 0;
	?>
						...
						<div class="row">
							...
							if( $tabbed_listings->have_posts() ) {
								while ( $tabbed_listings->have_posts() ) {
									...
									$listing_img = get_the_post_thumbnail( get_the_ID(), 'listingo_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
									$listing_address = ! empty( listingo_meta( 'listing_address') ) ? listingo_meta( 'listing_address') : 'N/A';
									$phone_number = ! empty( listingo_meta( 'phone_number') ) ? listingo_meta( 'phone_number') : 'N/A';
									$listing_email = ! empty( listingo_meta( 'listing_email') ) ? listingo_meta( 'listing_email') : 'N/A';
									?>
...
function set_custom_edit_listing_columns($columns) {
    ...
    $columns['listing_price'] = __( 'Listing Price', 'listingo_companion' );
    $columns['listing_area'] = __( 'Listing Area', 'listingo_companion' );
...
function custom_listing_column( $column, $post_id ) {
    ...
    switch ( $column ) {
        ...
        case 'listing_price' :
            echo get_post_meta( $post_id , '_listingo_listing_price' , true ); 
            break;

        case 'listing_area' :
            echo get_post_meta( $post_id , '_listingo_listing_area' , true ); 
            break;
...
if ( ! function_exists( 'search_prop_form_datas' ) ) {
	function search_prop_form_datas() {
		...
		if ( $search_text == '' || $search_category == '' || $search_location == '' || $search_area_from == '' || $search_area_to == '' || $price_min == '' || $price_max == '' ) {
			...
			$response['message']  = __( 'Sorry! Empty field is not allowed.', 'listingo-companion' );
		} else {
			$item = 0;
			...
			$properties = new WP_Query( array(
				...
				'meta_query' => array(
					'relation' => 'AND',
					array(
						'key'     => '_listingo_listing_price',
						'value'   => array($price_min, $price_max),
						'type'    => 'numeric',
						'compare' => 'BETWEEN',
					),
					array(
						'key'     => '_listingo_listing_area',
			...
			if( $properties->have_posts() ) {
				while ( $properties->have_posts() ) {
					$properties->the_post();		
					$property_img = get_the_post_thumbnail( get_the_ID(), 'listingo_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
